document.addEventListener("DOMContentLoaded", function() {
    ...
    async function loadStations() {
        try {
            ...
            if(stationFilterSelect) stationFilterSelect.innerHTML = '<option value="all">Tất cả điểm thuê</option>';
    ...
    if (addStaffForm) {
        addStaffForm.addEventListener('submit', async function(event) {
            ...
            try {
                const response = await fetch('/admin/staff/add', {
                    method: 'POST',
    ...
    if (addStaffBtn) addStaffBtn.onclick = () => { addStaffForm.reset(); addModal.style.display = "block"; }
    if (addCloseButton) addCloseButton.onclick = () => addModal.style.display = "none";
